...
function Form({ onComplete, className, item, mode = 'create', title = '' }: { onComplete?: () => void, className?: string, item?: ProcurementRow, mode?: 'create' | 'update', title: string }) {
    ...
    let fetch_url = `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/procurements`
    ...
    return (
        <section className={className}>
            ...
            <form onSubmit={handleSubmit(submit)} className="flex flex-col gap-4">
                <div>
                    <SelectCompany label="Company" {...register("company_id", {valueAsNumber: true})} />
                    <InputError message={errors.company_id?.message} className="mt-2" />
                </div>

                <div>
                    <SelectVendor company_id={watch("company_id")} label="Vendor" {...register("vendor_id")} />
                    <InputError message={errors.vendor_id?.message} className="mt-2" />
                </div>

                <div>
                <InputLabel htmlFor="name" value="Procurement Title" />
                <TextInputx
                    id="name"
